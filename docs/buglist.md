...
| RB-40 | **P1** | 服务 | 还原（RB-35 方案C）为了让 SYSTEM 能 rename 强行取得所有权，之后从未还回去——所有还原出来的文件/目录，属主永久变成 `NT AUTHORITY\SYSTEM`；且 `RestoreInheritParentAcl` 只重置了 DACL，原本独立于父目录的显式权限（针对特定用户/组的授权、deny 规则）被静默丢弃 | **已修复**（新增 `CaptureOwnerSid`/`RestoreOwnerSid`：在 `GrantSelfAccessToStaging` 抢所有权之前先记下原 Owner SID，rename + DACL 重置完成后再设回去；失败仅记警告，不影响本次还原成功与否） |
| RB-41 | P3 | 服务/Go API/Web | RB-40 只保证 Owner 一定还回去，DACL 默认仍是"继承父目录"（RB-35 的选择）——树形批量还原下这是对的（跟随共享当前权限策略），但单个文件如果原来有独立于父目录的显式授权，希望原样恢复时没有入口 | **已实现（可选项）**：`RestoreItemById` 新增 `preserveAcl` 参数，为真时在 `GrantSelfAccessToStaging` 之前用 `CaptureDacl` 记下原始 DACL，rename 完成后用 `RestoreDacl`（`PROTECTED_DACL_SECURITY_INFORMATION`，即显式非继承）原样写回；默认仍是 `RestoreInheritParentAcl`。链路：`db/schema.sql` 新增 `ops.preserve_acl` 列 → Go API `POST /ops` 接受 `preserve_acl` → `rbdb.c`/`rbsvc.h` 的 `RBSVC_OP.PreserveAcl` → `rbrestore.c`。Web 管理面板补了"保留原权限"勾选框（单文件还原按钮下方，见 RB-41b 关于目录的部分） |
| RB-41b | P3 | 服务/Go API/Web | RB-41 最初把 `RestoreTreeByPrefix` 排除在外，内部固定传 `preserveAcl=0`——树越大、脱离继承链的对象越多，代价被认为不该由批量操作隐式承担。实测场景反馈：**整棵目录（含所有子目录/文件）按原样权限恢复本身就是有价值的需求**，不应该完全不给入口 | **已实现（可选项，默认关闭）**：`RestoreTreeByPrefix` 新增 `preserveAcl` 参数，对树里每一个条目（文件和目录一视同仁）透传给 `RestoreItemById`，跟单文件走同一套 `CaptureDacl`/`RestoreDacl`；`RestoreDrainOps` 处理 `restore-tree` 时改传 `ops[i].PreserveAcl`（原来完全没用这个字段）。性能/继承脱钩的代价（见 `CaptureDacl` 函数头注释）依然存在，只是从"批量场景永远不能选"变成"默认不选，管理员可以主动选"。Web 面板"还原目录"按钮下方加了同款"保留原权限（含子项）"勾选框，勾选后二次确认弹窗会额外提示"恢复后不再跟随共享当前权限策略" |
| RB-42 | **P0**（编译不过） | Go API | 实现 RB-41 时 `service_go/db/db.go` 的 `EnqueueOp` 签名改成了 4 个参数（加了 `preserveAcl bool`），但 `service_go/api/api.go` 里 `handleOps`（`POST /ops` 的实际入口，两种 op 类型都走这里）的调用点 `s.DB.EnqueueOp(req.Type, req.ID, req.Arg)` 没有跟着改，只传了 3 个参数——**这段代码当时就编译不过**，`go build` 会报参数个数不匹配。等于 RB-41 从合入的那一刻起 `rbapi.exe` 根本没法构建出来，Web 面板上"保留原权限"勾选框即使加了也不可能生效 | **已修复**：调用点补上第 4 个参数 `req.PreserveAcl`；顺带把 `EnqueueOp` 上那条"restore-tree 应该在 API 层拒绝 preserve_acl=true"的过时注释改成了 RB-41b 之后的实际语义（两种类型都支持，默认 false） |
| RB-43 | P2 | 服务/Go API/Web/schema | 管理员绕开管理面板，直接在桌面原生回收站里还原或彻底删除（Shift+Delete）文件，`items` 表毫不知情——见此前讨论：`landed` 行永久卡住，`recycle_path` 指向的 `$R` 已经不存在，配额统计也会算错 | **已实现（管理员手动触发，非定时）**：核心对账算法 `ReconcileLanded`（`service_c/rbconcile.c`）当时已经写好——按 `recycle_path` 目录分组 `qsort`，每个 `$Recycle.Bin\<SID>\` 目录只枚举一次（`FindFirstFile`/`FindNextFile`），`bsearch` 判断每行的 `$R` 是否还在，缺失的再用一次 `GetFileAttributesW` 查 `orig_path_dos` 区分"外部还原"还是"外部清空"——但缺了打通全链路的部分，这次补上：① `db/schema.sql` 的 `ops.type` CHECK 约束加 `'reconcile'`，并在 `rbdb.c` 新增 `DbMigrateOpsTypeCheck`（`ALTER TABLE ops RENAME` + 用新 DDL 重建 + 数据迁移回填）——`CREATE TABLE IF NOT EXISTS` 对已存在的库不会自动生效，已部署的库不重建表就永远拒绝这个新 type；② `rbrestore.c` 的 `RestoreDrainOps` 新增对 `reconcile` 的分发，调用 `ReconcileLanded()`；③ Go 侧 `SupportedOps` 加 `reconcile`，`handleOps` 新增分支（不需要 `id`/`arg`）；④ Web 面板"回收项"页新增"刷新对账"按钮，提交后轮询任务、完成时自动刷新列表 |
| RB-44 | P2 | 服务 | RB-43 只修数据库状态，没管文件本身：管理员走原生桌面回收站还原的文件，落地时打的 `Hidden+System` 属性（`SetHiddenSystem`，见 RB-30）从没被摘掉——`RestoreItemById` 里清这两个属性的那段代码，原生还原完全不会经过。管理员反馈"还原回来的文件还是隐藏的、权限很奇怪" | **已修复（仅摘属性，不动权限）**：`ReconcileLanded` 判定为"外部还原"（在 `orig_path_dos` 发现文件）时，顺手对这个路径 `GetFileAttributesW`/`SetFileAttributesW` 清掉 `FILE_ATTRIBUTE_HIDDEN`/`FILE_ATTRIBUTE_SYSTEM`。DACL/Owner 故意不动——`GrantSelfAccessToStaging`（RB-35/40）只在 `RestoreItemById` 里才会跑，原生还原从没碰过这个文件的权限，此刻磁盘上的权限本来就是删除前那一份，原样未变；"权限很奇怪"是这份冻结快照跟当前目录策略不一致造成的观感，跟"是否严格保留原样"是同一枚硬币的两面，两者不可兼得，这次按"保留原样"实现，不重置成继承当前目录 |
